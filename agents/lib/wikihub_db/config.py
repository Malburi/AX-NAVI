...
# -- 담당자(발행자) 정보 -------------------------------------------------------
# 발행 기록에 "누가 올렸는지"를 사람 단위로 남기기 위해 받는다. 이 정보는 전적으로 선택이다 —
# 아무것도 주지 않으면 담당자 없이 그대로 발행하고, 사용자에게 강제로 묻지 않는다.
# 다만 담당자를 남기기로 했다면(어느 필드든 하나라도 값이 있으면) 회사명·사번·성명은 사람을
# 식별하는 키라 셋 다 있어야 한다. 소속·전화번호·이메일은 없으면 경고만 하고 진행한다.
# .env 에 한 번 저장해두면 다음 발행부터는 다시 묻지 않는다.
PUBLISHER_FIELDS = [
...
def has_publisher_input(person):
    """담당자 필드 중 하나라도 값이 있으면 True. 담당자 정보는 선택이라,
    아무 입력도 없을 때(전부 False) 발행을 강제로 막지 않기 위한 판별용이다."""
    return any((person.get(key) or "").strip() for key, *_ in PUBLISHER_FIELDS)


def validate_publisher(person):
    """형식 검사. 반환: 경고 문자열 목록. 담당자를 남기기로 했는데(어느 필드든 입력이 있는데)
    식별 키가 빠졌을 때만 예외를 던진다. 호출 측이 has_publisher_input() 로 입력 유무를
    먼저 확인하므로, 여기 왔다는 건 사용자가 담당자를 남기려 한 것이다."""
    import re
    ...
    if missing:
        raise ConfigError(
            ...
            "담당자를 남기지 않으려면 관련 값을 비워 두거나 --skip-publisher 를 쓰세요."
        )
